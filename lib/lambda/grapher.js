...
class Grapher {
  ...
  graphTree() {
    const title = Printer.print(this.ast, { names: this.names });
  ...
  label(node) {
    switch (node.type) {
      ...
      case "variable": return this.variableName(node);
  ...
  abstractionName(node) {
    return this.names ? "λ" + node.name : "λ";
  }

  variableName(node) {
    const index = node.index.toString();
    return this.names ? node.binder.name : index;
  }
  ...
  freeVarName(node) {
    ...
    return this.names ? node.name : index;
  }

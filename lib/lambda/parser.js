...
class Parser {
  static parse(tokens) {
    return new this(tokens).parse(true);
  }
  ...
  parse(topLevel) {
    ...
    while (this.tokens.length > 0) {
      const left = parent;
      const right = this.parsePrime(topLevel);
  ...
  parseParentheses() {
    ...
    this.validate(leftParen, "keyword", "(");

    const node = this.parse();

    const rightParen = this.read();
    this.validate(rightParen, "keyword", ")");
